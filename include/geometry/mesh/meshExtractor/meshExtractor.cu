// Copyright 2019 Alireza Ahmadi (Alireza.Ahmadi@uni-bonn.de), Cyrill Stachniss, University of Bonn
// Part of the code in this file was adapted from the original VoxelHashing
// implementation by Niessner et al and Refusion implemented by Emanuele Palazzolo (emanuele.palazzolo@uni-bonn.de).
...
void MeshExtractor::Init(unsigned int max_triangles, float voxel_size) {
  ...
  // cudaMallocManaged(&meshSTD, sizeof(meshSTD));
  // meshSTD->Init(900000, 300000);
...
__device__ 
bool TrilinearInterpolation(tsdf::tsdfVolume *volume, float voxel_size,
...
__device__ 
Vertex VertexInterpolation(float isolevel, const float3 &p1,
...
__device__ 
void ExtractMeshAtPosition(tsdf::tsdfVolume *volume,
  ...
  for (int i = 0; triTable[cubeindex][i] != -1; i += 3) {
    ...
    //printf("z: %f, y: %f, z: %f \n",t.v0.position.x,t.v0.position.y,t.v0.position.z);
    mesh->AppendTriangle(t);
  }
}

__device__ 
void ExtractMeshSTDAtPosition(tsdf::tsdfVolume *volume,
                                      const float3 &position, float voxel_size,
                                      MeshSTD *meshSTD, int idx) {
  const float isolevel = 0.0f;
  const float P = voxel_size/2.0f;
  const float M = -P;
  ...
  Vertex v0,v1,v2;
  for (int i = 0; triTable[cubeindex][i] != -1; i += 3) {   // todo...

    v0 = vertlist[triTable[cubeindex][i + 0]];
    v1 = vertlist[triTable[cubeindex][i + 1]];
    v2 = vertlist[triTable[cubeindex][i + 2]];

    // printf("z: %f, y: %f, z: %f \n",v0.position.x,v0.position.y,v0.position.z);
    meshSTD->appendVertices(v0, v1, v2, idx);
  }
}

__global__ 
void ExtractMeshKernel(tsdf::tsdfVolume *volume, float3 lower_corner,
  ...
  int3 grid_size =
      make_int3(size.x / voxel_size* 2, size.y / voxel_size* 2, size.z / voxel_size * 2);
...
void MeshExtractor::ExtractMesh(tsdf::tsdfVolume *volume, float3 lower_corner,
                                float3 upper_corner) {
  int threads_per_block = 32;
  int thread_blocks =
  (volume->GetProperties().numBlocks * volume->GetProperties().blockSize +
  threads_per_block - 1) /
  threads_per_block;
  //std::cout << "<<<ExtractMesh>>> threadBlocks: "<< thread_blocks << ", threadPerBlock: " << threads_per_block << std::endl;
